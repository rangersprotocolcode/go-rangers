...
func initBlockChain(helper types.ConsensusHelper) error {
	...
	chain.blocks, err = db.NewDatabase(chain.config.block)
	if err != nil {
		//todo: 日志
	...
	chain.blockHeight, err = db.NewDatabase(chain.config.blockHeight)
	if err != nil {
		logger.Debugf("Init block chain error! Error:%s", err.Error())
		//todo: 日志
		return err
	...
	chain.statedb, err = db.NewDatabase(chain.config.state)
	if err != nil {
		logger.Debugf("Init block chain error! Error:%s", err.Error())
		//todo: 日志
		return err
	...
	chain.checkdb, err = db.NewDatabase(chain.config.state)
	if err != nil {
		logger.Debugf("Init block chain error! Error:%s", err.Error())
		//todo: 日志
		return err
...
func Clear() {
	path := db.DEFAULT_FILE
	if nil != common.GlobalConf {
		path = common.GlobalConf.GetString(CONFIG_SEC, "database", db.DEFAULT_FILE)
	}
	os.RemoveAll(path)
}

func (chain *FullBlockChain) GetAccountDBByHash(hash common.Hash) (*account.AccountDB, error) {
	header := chain.QueryBlockHeaderByHash(hash)
	return account.NewAccountDB(header.StateTree, chain.stateCache)
}

func (chain *FullBlockChain) GetAccountDBByHeight(height uint64) (*account.AccountDB, error) {
