...
type testMinerInfo struct {
	AccountPrivateKey common.PrivateKey
...
func createGroupMembers(groupMemberNum uint64, pks []string) []*testMinerInfo {
	...
	for ; i < groupMemberNum; i++ {
		...
		miner.SecretSeed = base.RandFromBytes(miner.AccountPrivateKey.PrivKey.D.Bytes())
...
func TestGenComparisonData(test *testing.T) {
	...
	for i := 0; i < testCount; i++ {
		...
		threshold := int(math.Ceil(float64(groupMemberNum*51) / 100))
		buffer.WriteString("privateKeyList:")
		for index, member := range groupMemberList {
			buffer.WriteString(common.Bytes2Hex(member.AccountPrivateKey.PrivKey.D.Bytes()))
...
func TestValidateComparisonData(test *testing.T) {
	...
	for _, record := range records {
		...
		privateKeyListStr := strings.Replace(elements[0], "privateKeyList:", "", 1)
		privateKeyList := strings.Split(privateKeyListStr, "&")
		...
		validateFunction(privateKeyList, groupPublicKey, message, groupSign, test)
	}
}

func validateFunction(privateKeyList []string, groupPublicKey, messageStr, groupSign string, test *testing.T) {
	//validate groupPublicKey
	groupMemberList := make([]*testMinerInfo, 0)
	for _, privateKey := range privateKeyList {
		miner := testMinerInfo{}
		miner.AccountPrivateKey = *common.HexStringToSecKey(privateKey)
		id := miner.AccountPrivateKey.GetPubKey().GetID()
		miner.ID.Deserialize(id[:])
