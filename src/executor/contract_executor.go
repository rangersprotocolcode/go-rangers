...
func (this *contractExecutor) Execute(transaction *types.Transaction, header *types.BlockHeader, accountdb *account.AccountDB, context map[string]interface{}) (bool, string) {
	...
	if transaction.Target == "" {
		...
		this.logger.Tracef("hash: %s, source: %s, target: %s, data: %s, nonce: %d, nonce: %d, contract: %s", transaction.Hash.String(), vmCtx.Origin.String(), transaction.Target, transaction.Data, nonce1, accountdb.GetNonce(caller.Address()), contractAddress.String())
		...
		this.logger.Tracef("hash: %s, source: %s, target: %s, data: %s, nonce: %d, nonce: %d", transaction.Hash.String(), vmCtx.Origin.String(), transaction.Target, transaction.Data, nonce1, accountdb.GetNonce(caller.Address()))
