...
func ECVRF_prove(pk PublicKey, sk PrivateKey, m []byte) (pi VRFProve, err error) {
	x := expandSecret(sk)
...
func ECVRF_hash_to_curve(m []byte, pk PublicKey) *edwards25519.ExtendedGroupElement {
	hash := sha256.New()
...
func expandSecret(sk PrivateKey) *[32]byte {
