...
// secp256k1_context_create_vm_sign_verify creates a context for signing and signature verification.
static secp256k1_context* secp256k1_context_create_vm_sign_verify() {
	return secp256k1_context_create_vm(SECP256K1_CONTEXT_SIGN | SECP256K1_CONTEXT_VERIFY);
}
...
static int secp256k1_ext_ecdsa_recover(
	...
	secp256k1_ecdsa_recover_vmable_signature sig;
	secp256k1_pubkey pubkey;

	if (!secp256k1_ecdsa_recover_vmable_signature_parse_compact_vm(ctx, &sig, sigdata, (int)sigdata[64])) {
		return 0;
	}
	if (!secp256k1_ecdsa_recover_vm(ctx, &pubkey, &sig, msgdata)) {
		return 0;
	}
	size_t outputlen = 65;
	return secp256k1_ec_pubkey_serialize_vm(ctx, pubkey_out, &outputlen, &pubkey, SECP256K1_EC_UNCOMPRESSED);
}
...
static int secp256k1_ext_ecdsa_verify(
	...
	if (!secp256k1_ecdsa_sign_vmature_parse_compact_vm(ctx, &sig, sigdata)) {
		return 0;
	}
	if (!secp256k1_ec_pubkey_parse_vm(ctx, &pubkey, pubkeydata, pubkeylen)) {
		return 0;
	}
	return secp256k1_ecdsa_verify_vm(ctx, &sig, msgdata, &pubkey);
}
...
static int secp256k1_ext_reencode_pubkey(
	...
	return secp256k1_ec_pubkey_serialize_vm(ctx, out, &outlen, &pubkey, flag);
}

// secp256k1_ext_scalar_mul_vm multiplies a point by a scalar in constant time.
//
...
int secp256k1_ext_scalar_mul_vm(const secp256k1_context* ctx, unsigned char *point, const unsigned char *scalar) {
	int ret = 0;
	int overflow = 0;
