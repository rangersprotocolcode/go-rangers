...
do
	...
	if [ $instance_index -le 3 ];then
		nohup env GOTRACEBACK=crash ./rangers_node miner --config $config_file --rpc --rpcport $rpc_port  --instance $instance_index --pprof $pprof_port  --gateaddr 'ws://gate.tuntunhz.com:8899' --outergateaddr 'ws://gate.tuntunhz.com:8888' --mysql='readonly:Tuntun123456!@tcp(api.tuntunhz.com:3336)/rpservice_dev?charset=utf8&parseTime=true&loc=Asia%2FShanghai' --env dev> $stdout_log 2>&1 & echo $! > $pid_file
