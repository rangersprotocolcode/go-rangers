...
do
	...
	if [ $instance_index -le 3 ];then
		nohup env GOTRACEBACK=crash ./rocket_node miner --config $config_file --rpc --rpcport $rpc_port  --instance $instance_index --pprof $pprof_port  --gateaddr gate.tuntunhz.com:8888 --env dev> $stdout_log 2>&1 & echo $! > $pid_file
